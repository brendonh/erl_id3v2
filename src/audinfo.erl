...
test_file(FN) ->
    {Artist, Album, Title, Track} = get_info(FN),
    io:format("~.30s~.30s~.70s~s~n", [adjust_name(Artist),
                                      adjust_name(Album), 
                                      adjust_name(Title), 
                                      adjust_track(Track)]).
...
get_info(Filename) ->
    ...
    {match, [Path, Ext]} = re:run(Filename, "^(.*)[.]([a-zA-Z0-9]+)$", [{capture, all_but_first, list}]),

    {Title, Artist, Album, Track} = get_metadata(Ext, Filename),
    ...
    {merge(Title, Title2), merge(Artist, Artist2), merge(Album, Album2), merge(Track, Track2)}.
...
get_metadata("mp3", Filename) ->
    case id3v2:read_file(Filename) of
        {ok, P} -> 
            {?GV(tpe1, P), ?GV(talb, P), ?GV(tit2, P), get_track(?GV(trck, P))};
        not_found -> 
