...
-export([read_file/1, read/1, test/0, parse_synchsafe/1]).
...
read_next_frame_24(<<0, Rest/binary>>, Frames) ->
...
read_next_frame_23(<<0, Rest/binary>>, Frames) ->
    % Assume the header length was a lie, and we're done
    lists:reverse(Frames);
...
read_v1(File, Props) ->
    file:position(File, {eof, -125}),
    {ok, Header} = file:read(File, 125),
    <<Title:30/binary, Artist:30/binary, Album:30/binary, Year:4/binary,
     Comment:30/binary, Genre:1/binary>> = Header,
    Frames = [{title, Title}, {artist, Artist}, {album, Album},
              {year, Year}, {comment, Comment}, {genre, Genre}],
    %?DBG({id3v1, Frames}),
    {ok, [{frames, Frames}|Props]}.
